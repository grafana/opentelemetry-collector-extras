...
processors:
  spanpruning:
    ...
    # When enabled, records bytes_received, bytes_processed_input, bytes_processed_output, and bytes_emitted metrics
    # This serializes each batch (full batch plus matched subset before/after pruning) and is expensive
    # Default: false
...
**Trace-level “matched” scope:** OTTL `conditions` mark a *trace* as matched when *any* span matches. `bytes_processed_input` and `bytes_processed_output` include **all spans in those traces**, not only the spans that satisfied the condition.

| Comparison | Valid? |
|------------|--------|
| `bytes_processed_input` vs `bytes_processed_output` | Yes — same matched trace set, before vs after pruning |
| `bytes_received` vs `bytes_emitted` | Yes — full batch before vs after pruning |
| `bytes_emitted` vs `bytes_processed_input` | **No** — batch vs matched subset; ratios can be 100×+ |

**Matched-byte savings** (percentage of bytes removed from the matched subset):

```promql
(1 - (
  sum(rate(otelcol_processor_spanpruning_bytes_processed_output_total[5m]))
  /
  sum(rate(otelcol_processor_spanpruning_bytes_processed_input_total[5m]))
)) * 100
```

After aggregation, `bytes_processed_output` can exceed `bytes_processed_input` when summary spans are larger than the leaf spans they replace, so this ratio may be **negative** even when pruning is working.
...
- Track matched-byte compression with `bytes_processed_input` vs `bytes_processed_output` (see [Byte metrics semantics](#byte-metrics-semantics))
